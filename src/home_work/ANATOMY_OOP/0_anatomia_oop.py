...
class Танк:
    def __init__(self):
        self.экипаж = 12      #Public   # ДОСТУПЕН ВСЕМ
        self._топливо = 50    #Protected# ДОСТУПЕН Т! СЕБЕ И ДОЧЕРНИМ КЛАССАМ
        self.__боезапас = 100 #Privat   # ДОСТУПЕН Т! СЕБЕ - Приватный атрибут
...
#                   имен.
class Outer:
    var_outer = 'Внешняя переменная'

    class Inner:
        var_inner = 'Внутренняя переменная'

print(Outer.var_outer)     # Вывод: Внешняя переменная
print(Outer.Inner.var_inner)  # Вывод: Внутренняя переменная
...
### 31
# ПРОТОКОЛЫ И - duck typing - (Утиный ввод)
