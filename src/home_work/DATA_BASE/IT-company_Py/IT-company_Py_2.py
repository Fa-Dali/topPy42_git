...
class DBManager:
    ...
    def close(self):
        ...
        pass  # Этот метод не нужен, так как соединение закрывается в execute_query и fetch_all
...
class Customer(DBManager):
    ...
    def create_customer(self, firm_name, contact_person, phone_number):
        ...
        query = """INSERT INTO customers (firm_name, contact_person, phone_number) VALUES (%s, %s, %s);"""
        self.execute_query(query, (firm_name, contact_person, phone_number))  # Выполнение запроса
...
class Project(DBManager):
    ...
    def create_project(self, customer_id, start_date, end_date, manager_id,
                       title):
        query = """INSERT INTO projects (customer_id, 
                                        project_start, 
                                        project_end, 
                                        project_manager_id, 
    ...
    def read_projects(self):
        # Добавление нового проекта в таблицу projects
        query = """
            SELECT p.id, 
                    c.firm_name, 
                    to_char(p.project_start, 'DD.MM.YYYY'), 
                    to_char(p.project_end, 'DD.MM.YYYY'),
                    pm.full_name, 
                    p.title
            FROM projects p
            JOIN customers c ON p.customer_id = c.id
    ...
    def update_project(self, project_id, new_data):
        # Обновление данных проекта в таблице project
        query = """
            UPDATE projects SET customer_id=%s, 
                                project_start=%s, 
                                project_end=%s, 
    ...
    def delete_project(self, project_id):
        # Удаление проекта из таблицы project
...
class ProjectManager(DBManager):
    ...
    def create_manager(self, birth_date, full_name, phone_number):
        query = """INSERT INTO project_managers (birth_date, 
                                full_name, 
                                phone_number) VALUES (%s, %s, %s);"""
        self.execute_query(query, (birth_date,
                                   full_name,
                                   phone_number))
    ...
    def update_manager(self, manager_id, new_data):
        query = """UPDATE project_managers SET birth_date=%s, 
                                                full_name=%s, 
                                                phone_number=%s WHERE id=%s;"""
        self.execute_query(query, (
...
class Task(DBManager):
    ...
    def create_task(self, project_id, start_time, end_time, programmer_id,
                    description):
        query = """INSERT INTO tasks (project_id, 
                                        task_start, 
                                        task_end, 
...
class Programmer(DBManager):
    ...
    def create_programmer(self, birth_date, full_name, skill_level,
                          phone_number):
        query = """INSERT INTO programmers (birth_date, 
                                            full_name, 
                                            skill_level, 
    ...
    def update_programmer(self, programmer_id, new_data):
        query = """UPDATE programmers SET birth_date=%s, 
                                        full_name=%s, 
                                        skill_level=%s, 
                                        phone_number=%s WHERE id=%s;"""
        self.execute_query(query, (new_data['birth_date'],
                                   new_data['full_name'],
                                   new_data['skill_level'],
